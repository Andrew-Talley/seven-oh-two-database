...
CREATE TABLE IF NOT EXISTS `project2`.`Tournament` (
  ...
  `first_coin_flip` VARCHAR(45) NULL CHECK (first_coin_flip IN ('Heads', 'Tails')),
  `second_coin_flip` VARCHAR(45) NULL CHECK (second_coin_flip IN ('Heads', 'Tails')),
...
CREATE TABLE IF NOT EXISTS `project2`.`Student` (
  ...
  `role` VARCHAR(8) NOT NULL CHECK (role IN ('Attorney', 'Witness')),
  `ranks` TINYINT UNSIGNED NOT NULL,
  `side` VARCHAR(8) NOT NULL CHECK (side IN ('π', '∆')),
...
CREATE TABLE IF NOT EXISTS `project2`.`CaseDetails` (
  ...
  `type_charge` VARCHAR(10) NULL CHECK (type_charge IN ('Criminal', 'Civil')),
...
CREATE TABLE IF NOT EXISTS `project2`.`WitnessDetails` (
  ...
  `witness_type` VARCHAR(45) NULL CHECK (witness_type IN ('Character', 'Police Officer', 'Expert', 'Defendant', 'Party Representative')),
  `witness_side` VARCHAR(8) NULL CHECK (witness_side IN ('π', '∆', 'Swing')),
